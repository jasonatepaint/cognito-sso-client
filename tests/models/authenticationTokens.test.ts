...
import { getLocalStorageWithExpiration, removeFromLocalStorage, setLocalStorageWithExpiration } from "../../src/storage";
...
describe('Authentication Tokens', () => {
	...
	describe('Set Tokens', () => {
		...
		test('as expected', async () => {
			...
			expect(setLocalStorageWithExpiration).toHaveBeenCalledWith(AUTH_ACCESS_STORAGE_KEY, authentication.accessToken, expiration);
			expect(setLocalStorageWithExpiration).toHaveBeenCalledWith(AUTH_ID_STORAGE_KEY, authentication.idToken, expiration);

			expiration = 60 * 60 * REFRESH_TOKEN_EXPIRATION_DAYS;
			expect(setLocalStorageWithExpiration).toHaveBeenCalledWith(AUTH_REFRESH_STORAGE_KEY, authentication.refreshToken, expiration);
		});
		...
		test('missing idToken', async () => {
			...
			expect(setLocalStorageWithExpiration).toHaveBeenCalledWith(AUTH_ACCESS_STORAGE_KEY, authentication.accessToken, expiration);
			expect(setLocalStorageWithExpiration).not.toHaveBeenCalledWith(AUTH_ID_STORAGE_KEY, expect.anything(), expect.anything());
			expect(setLocalStorageWithExpiration).toHaveBeenCalledWith(AUTH_REFRESH_STORAGE_KEY, authentication.refreshToken, expect.anything());
		});
		...
		test('missing refreshToken', async () => {
			...
			expect(setLocalStorageWithExpiration).toHaveBeenCalledWith(AUTH_ACCESS_STORAGE_KEY, authentication.accessToken, expiration);
			expect(setLocalStorageWithExpiration).toHaveBeenCalledWith(AUTH_ID_STORAGE_KEY, authentication.idToken, expiration);
			expect(setLocalStorageWithExpiration).not.toHaveBeenCalledWith(AUTH_REFRESH_STORAGE_KEY, expect.anything(), expect.anything());
		});
	});
