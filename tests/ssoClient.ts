...
describe('Initialize', () => {
	...
	test('callback is not a function', async () => {
		let error;
		try {
			SsoClient.initialize(ssoUrl, clientId, redirectUri, authFrame, config.options, <any>{ data: "not a function" });
		} catch (e) {
			error = e;
		}
...
describe('Handle User Activity', () => {
	...
	describe('Last Token Check', () => {
		...
		test('enough time as elapsed', async () => {
			getTestContext().setLastTokenCheck(dayjs(date).subtract(TOKEN_CHECK_INTERVAL_SECONDS + 1, "second").toDate().getTime());
			let updated = handleUserActivity();
			expect(updated).toBeTruthy();
	...
	test('token expiration within buffer range', async () => {
		...
		mockParseToken.mockReturnValue({
			exp: dayjs().add(TOKEN_CHECK_INTERVAL_SECONDS + tokenExpirationBufferSeconds - 1, "second").unix()
		});
	...
	test('token expiration not in buffer range', async () => {
		...
		mockParseToken.mockReturnValue({
			exp: dayjs().add(TOKEN_CHECK_INTERVAL_SECONDS + tokenExpirationBufferSeconds + 10, "second").unix()
		});
